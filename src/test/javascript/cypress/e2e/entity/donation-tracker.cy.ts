...
describe('DonationTracker e2e test', () => {
  ...
  describe('new DonationTracker page', () => {
    ...
    it('should create an instance of DonationTracker', () => {
      cy.get(`[data-cy="branchCode"]`).type('now');
      cy.get(`[data-cy="branchCode"]`).should('have.value', 'now');

      cy.get(`[data-cy="donationIdCode"]`).type('whoever');
      cy.get(`[data-cy="donationIdCode"]`).should('have.value', 'whoever');

      cy.get(`[data-cy="date"]`).type('2026-06-11');
      cy.get(`[data-cy="date"]`).blur();
      cy.get(`[data-cy="date"]`).should('have.value', '2026-06-11');

      cy.get(`[data-cy="donorNameOrOrg"]`).type('glass tray um');
      cy.get(`[data-cy="donorNameOrOrg"]`).should('have.value', 'glass tray um');

      cy.get(`[data-cy="contactDetails"]`).type('following');
      cy.get(`[data-cy="contactDetails"]`).should('have.value', 'following');

      cy.get(`[data-cy="amount"]`).type('13034.12');
      cy.get(`[data-cy="amount"]`).should('have.value', '13034.12');

      cy.get(`[data-cy="purpose"]`).type('cheerfully yahoo');
      cy.get(`[data-cy="purpose"]`).should('have.value', 'cheerfully yahoo');

      cy.get(`[data-cy="receivedViaMode"]`).select('CHEQUE');

      cy.get(`[data-cy="notes"]`).type('notarize extra-large');
      cy.get(`[data-cy="notes"]`).should('have.value', 'notarize extra-large');
