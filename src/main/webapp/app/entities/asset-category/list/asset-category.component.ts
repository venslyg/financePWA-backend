...
import { AssetCategoryService, EntityArrayResponseType } from '../service/asset-category.service';
...
export class AssetCategoryComponent implements OnInit {
  ...
  assetCategories = signal<IAssetCategory[]>([]);
  isLoading = false;
  ...
  protected readonly assetCategoryService = inject(AssetCategoryService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);
  protected ngZone = inject(NgZone);

  trackId = (item: IAssetCategory): number => this.assetCategoryService.getAssetCategoryIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && AssetCategoryComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && AssetCategoryComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.assetCategories.set(dataFromBody);
  }
